...
## D-056 · 2026-07-07 · Registration flow: OTP typed in-app, template versioned, local auth mirrors hosted (LCHP-29)

**Decision:** the anonymous→registered upgrade (D-055's choice of OTP over clickable link) is implemented as `updateUser({ email })` → 6-digit code → `verifyOtp({ type: 'email_change' })`, entirely inside the app. Three implementation rulings: **(a) the email template is code** — `supabase/templates/email_change.html` (Spanish, renders `{{ .Token }}`) is wired into `config.toml` for the local stack and is the reference the hosted template must mirror (dashboard or Management API — a PRE-merge step, exceptionally: Codex round 3 flagged that deploying before the template renders {{ .Token }} would send the default link-only email with no typable code, and patching early is zero-risk because nothing in production sends email_change emails until this branch deploys); **(b) local auth config mirrors hosted**: `enable_confirmations = true`, because with the local default (autoconfirm) GoTrue applies email changes INSTANTLY and the entire OTP step the app depends on never runs locally — the first live check "passed" without ever sending an email; **(c) OTP length is 6** on both sides (hosted default was 8) — shorter for non-technical neighbors — but the panel's input tolerates 8 digits and only gates at ≥6 (Codex review, HIGH: Cloudflare deploys main on merge while the hosted `otp_length` change is manual, and a 6-capped input would make 8-digit production codes untypeable during that window). The local email rate limit is raised to 100/h (everything lands in Mailpit); the hosted one stays at 2/h until LCHP-31 (custom SMTP, pre-pilot blocker).
**Alternatives:** keep autoconfirm locally (hides the real flow; the live check proved it lies) · clickable magic link (breaks in installed iOS PWAs — D-055 research) · leave hosted at 8 digits (mismatched UI and longer for older users).
**Why:** the flow was verified end-to-end against real GoTrue + Mailpit: provisional confirmation while anonymous → code arrives with the Spanish subject → verify → same uid registered → LCHP-15's trigger pays the +5 retroactively and validates the sighting. A wrong code answers 403.
**Trail:** LCHP-29 · src/lib/registration.ts · src/features/registration/RegistrationPanel.tsx · supabase/templates/email_change.html · supabase/config.toml · LCHP-31 (SMTP) · D-054, D-055.
