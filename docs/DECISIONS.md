...
**Decision:** Cloudflare Pages "Build watch paths" is set to **Include-only**: `src/**`, `public/**`, `index.html`, `vite.config.ts`, `package.json`, `pnpm-lock.yaml`, `tsconfig*.json`. Commits that touch only documentation (`docs/`, `*.md`, `.github/`, this log) no longer trigger a Cloudflare build or deployment — neither production nor PR preview. GitHub Actions (Quality build·lint·format·test + gitleaks) still run on every commit: watch paths are a Cloudflare-only setting and do not touch the required merge checks.
